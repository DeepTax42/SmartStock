...
export function ChatBot() {
  ...
  const analyzeData = (data) => {
    ...
    data.forEach((item) => {
      ...
      productMap[productName].availables.push(
        parseFloat(item[availableCol]) || 0
      );
      productMap[productName].scheduleds.push(
        parseFloat(item[scheduledCol]) || 0
      );
    ...
    const analysis = Object.keys(productMap).map((productName) => {
      ...
      const avgAvailable =
        data.availables.reduce((a, b) => a + b, 0) / data.availables.length;
      const minAvailable = Math.min(...data.availables);
      const maxStock = Math.max(...data.stocks);
      const avgScheduled =
        data.scheduleds.reduce((a, b) => a + b, 0) / data.scheduleds.length;

      const firstStock = data.stocks[0];
      const lastStock = data.stocks[data.stocks.length - 1];
      const stockTrend =
        firstStock > 0 ? ((lastStock - firstStock) / firstStock) * 100 : 0;
    ...
    return {
      ...
      insights: {
        ...
        lowStock: analysis.filter(
          (i) => i.status === "부족" || i.status === "주의"
        ).length,
        avgStockLevel: Math.round(
          analysis.reduce((sum, i) => sum + i.avgStock, 0) / analysis.length
        ),
        totalScheduled: Math.round(
          analysis.reduce((sum, i) => sum + i.avgScheduled, 0)
        ),
      },
    };
  };
  ...
  const generateAIResponse = async (query) => {
    ...
    const sample = analyzedData.items
      ...
      .map(
        (i) =>
          `${i.name}: 평균재고 ${i.avgStock}, 최소가용 ${i.minAvailable}, 상태 ${i.status}`
      )
      .join("\n");
          ...
          <div className="flex items-center justify-between p-4 border-b bg-blue-600 text-white rounded-t-lg">
            <div>
              ...
              <p className="text-xs opacity-90">
                {analyzedData
                  ? `✅ ${analyzedData.insights.totalItems}개 상품 분석됨`
                  : "⚡ 지능형 분석 모드"}
              </p>
            </div>
            <Button
